    ...
    public class UserService : IUserService
    {
        ...
        public async Task<UserResponseDTO> CreateSystemUser(CreateSystemUserDTO createSystemUserDto, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                // Sử dụng using để đảm bảo TransactionScope được dispose đúng cách
                using (var scope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    if (result <= 0)
                    {
                        _logger.LogError("Failed to add user to database. Result: {Result}", result);
                        throw new Exception("Failed to add user to database.");
                    }

                    // Đánh dấu giao dịch là thành công
                    scope.Complete();
                    _logger.LogInformation("User {Email} created successfully and transaction committed.", user.Email);
            ...
            {
                _logger.LogError(ex, "Error adding new system user: {ErrorMessage}", ex.Message);
                throw new ApplicationException("Error while saving user: " + ex.InnerException?.Message ?? ex.Message, ex);
            }
        }
